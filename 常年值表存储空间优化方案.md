...
alias: 
---
todo[[气象局@西安中心数据迁移 2022-11-25.excalidraw]]
- [x] test 单机表的计划
	```sql
...
1. 创建一个原表的分布式备份表
	- 定义分布式dist_const_surf_bcccpsv3_glb_mul_hh_tab. **用他们的用户建表**
		- 选择分布列.
			- [ ] 确认业务sql
			- [ ] 确认数据分布
	- `copy dist_const_surf_bcccpsv3_glb_mul_hh_tab from nas`.
	- **查询dist表中sequence的last_value,set_value**
	- 比较数据量(分布式表、与复制表).
2. test sql
	- 在原表表上测试查询.
	- 在分布式备份表上测试查询.
	- 对比查询性能.
3. switch table name. 交换原表与备份表的名称, 同时保留两个表.
	```sql
	BEGIN;
	...
	-- grant all on const_nafp_cfsv2_glb_mul_hh_tab to user_sod;
	COMMIT;
	```
4. **查询dist表中sequence的last_value,set_value**
5. **切换后需要 用客户的用户查count( * );**
6. 观察业务在新表上的运行状况.观察一段时间业务在新表上的运行状况(1周以上), 确认无明显问题可以删除旧的复制表.
```sql
babj_pgdb=# select last_value from const_surf_bcccpsv3_glb_mul_day_d_record_id_seq;
 last_value
------------
    4282498
(1 行记录)
