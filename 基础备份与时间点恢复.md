...
- 忽略部分文件
	- `pg_wal`
	- `pg_repslot`
	-  `pg_dynshmem/`, `pg_notify/`, `pg_serial/`, `pg_snapshots/`, `pg_stat_tmp/`, and `pg_subtrans/` postmaster启动时生成
	- `pgsql_tmp` postmaster启动时移除
	- `pg_internal.init`
# 2. 时间点恢复（PITR）的工作原理
### A.过程
...
通过这种方式，每个时间线历史文件都会告诉我们每个恢复所得的数据库集簇的完整历史。它也在PITR过程中也有使用。
