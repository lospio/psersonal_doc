...
- 备机查询功能时需要修改参数：
	...
	- hot_standby_feedback可以避免删除最近失效的行。但是可能导致延迟清理，table bloat
	- vacuum_defer_cleanup_age不会立即清理失效的行 ，在主机设置
#### pigsty olap
- max_connections 100
- shard_buffers min(ceil(memory/400) * 100 ,64000) MB
- maintenance_work_mem ceil(shard_buffers / 20) * 10 MB 
- work_mem min( max(effective_cache_size / 3 / max_connections, 16), 1024)  MB
- max_prepared_transactions = if citus max_connections else 0
- max_locks_per_transactions if citus or timescaledb  16 * max_connections else 4 *  max_connections
- max_worker_process max(cpu,8)
- max_parallel_workers max(cpu,8)
- max_parallel_workers_per_gather ceil(max(cpu,8) / 2) 向上取整
- max_parallel_mt_workers max(ceil(cpu/8),2)
- huge_pages: try
- temp_file_limit : 1/10 disk size
- superuser_reserved_connections: 10
- wal_keep_size:  1/10 disk size
- max_wal_senders: 24                
...
- min_wal_size: 1/20 disk size
- max_wal_size: 1/5 disk size
- max_slot_wal_keep_size: 3/10 disk size 
- wal_buffers: 16MB
...
- random_page_cost:  4 for HDD, 1.1 for SSD
- effective_io_concurrency: 2 for HDD, 1000 for SSD
- effective_cache_size:  (max_em - shared_buffers) MB
- default_statistics_target: 1000         # stat bucket 100 -> 1000
#### REPORTING AND LOGGING
- log_destination: csvlog                 
- logging_collector: on                   
- log_filename: 'postgresql-%Y-%m-%d.log'   # log_filename: 'postgresql-%a.log'      weekly auto-recycle
- log_checkpoints: on                     
- log_lock_waits: on                      
...
# Note      :   Analysis Database Cluster Template (4-128 cores)
...
#==============================================================#
node_cpu:    "{{echo $(grep -c '^processor' /proc/cpuinfo)|int}}"

node_pages:    "{{getconf _PHYS_PAGES}}"

node_page_size:    "{{getconf PAGE_SIZE}}"

node_mem_mb:    "{{ (node_pages|int * node_page_size|int / 1048576) | round(0, 'ceil')|int }}"

node_fs_bytes:    "{{echo $(df $PGDATA --o=size | tail -1)}}|int * 1024"

{% set pg_max_connections = 100 %}

{% set pg_shared_buffers = ([(node_mem_mb|int / 400 | round(0, 'ceil'))|int * 100 , 64000])|min %}

{% set pg_maintenance_mem = ((pg_shared_buffers|int / 20)|round(0, 'ceil')|int * 10)|int %}

{% set pg_effective_cache_size = node_mem_mb|int - pg_shared_buffers|int  %}

{% set pg_workmem =  ([ ([ (pg_effective_cache_size / 3.0 / pg_max_connections)|round(0,'floor')|int , 16 ])|max|int , 1024])|min|int %}
...
{% set pg_max_locks_per_transaction = (16 * pg_max_connections)|int if 'citus' in pg_libs or 'timescaledb' in pg_libs else 4 * pg_max_connections %}

{% set pg_max_worker_processes = ([node_cpu|int, 8])|max|int %}

{% set pg_max_parallel_workers = pg_max_worker_processes|int %}

{% set pg_max_parallel_workers_per_gather = (pg_max_parallel_workers / 2)|round(0, 'ceil')|int %}
...
        temp_file_limit: {{ pg_size_twentieth * 2 }}GB     # 1/10 of disk size
