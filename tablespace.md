...
To define a tablespace, use the [CREATE TABLESPACE](https://www.postgresql.org/docs/10/manage-ag-tablespaces.htmlsql-createtablespace.html "CREATE TABLESPACE") command, for example::
...
Alternatively, use the [default\_tablespace](https://www.postgresql.org/docs/10/manage-ag-tablespaces.htmlruntime-config-client.html#GUC-DEFAULT-TABLESPACE) parameter:
...
There is also a [temp\_tablespaces](https://www.postgresql.org/docs/10/manage-ag-tablespaces.htmlruntime-config-client.html#GUC-TEMP-TABLESPACES) parameter, which determines the placement of temporary tables and indexes, as well as temporary files that are used for purposes such as sorting large data sets. This can be a list of tablespace names, rather than only one, so that the load associated with temporary objects can be spread over multiple tablespaces. A random member of the list is picked each time a temporary object is to be created.
...
To remove an empty tablespace, use the [DROP TABLESPACE](https://www.postgresql.org/docs/10/manage-ag-tablespaces.htmlsql-droptablespace.html "DROP TABLESPACE") command.

To determine the set of existing tablespaces, examine the [`pg_tablespace`](https://www.postgresql.org/docs/10/manage-ag-tablespaces.htmlcatalog-pg-tablespace.html "51.54. pg_tablespace") system catalog, for example

```
SELECT spcname FROM pg_tablespace;
```

The [psql](https://www.postgresql.org/docs/10/manage-ag-tablespaces.htmlapp-psql.html "psql") program's `\db` meta-command is also useful for listing the existing tablespaces.

PostgreSQL makes use of symbolic links to simplify the implementation of tablespaces. This means that tablespaces can be used _only_ on systems that support symbolic links.
